...
# todo modify events to contain portfolio name so that orders and fills can be attributed to the right sub portfolio
class MultiPortfolioHandler(object):
    """
    variation of the portfolio handler that can handle multiple sub portfolios
    ie a portfolio for each day of the week
    """
    def __init__(
        self, initial_cash, events_queue,
        price_handler, position_sizer, risk_manager, portfolios_names=None
    ):
        """
        ...
        self.portfolio = {}
        self.portfolio_names = portfolios_names
        for name in self.portfolio_names:
            self.portfolio[name] = Portfolio(price_handler, initial_cash/(len(self.portfolio_names)))
